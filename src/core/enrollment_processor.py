...
def preprocess_image_mobilefacenet(face_roi):
    ...
    img_resized = cv2.resize(
        face_roi, MOBILEFACENET_INPUT_SIZE, interpolation=cv2.INTER_AREA
    )
    # Convert to float32 and normalize to [-1, 1]
    img_normalized = (img_resized.astype(np.float32) / 127.5) - 1.0
    return img_normalized[np.newaxis, :, :, :]  # Add batch dimension
...
class AnchorBoxes:
    ...
    def _generate_anchors(self):
            ...
            for y in range(fm_h):
                for x in range(fm_w):
                    ...
                    if layer_idx < 3:  # First three layers (16x16, 8x8, 4x4)
                        ...
                        anchors.append(
                            [center_x, center_y, scale_var_1, scale_var_1]
                        )  # Aspect 1:1
                        anchors.append(
                            [center_x, center_y, scale_var_2, scale_var_2]
                        )  # Another aspect 1:1 or slightly different scale
        ...
        if final_anchors.shape[0] != 896:
            raise ValueError(
                f"Anchor generation failed: Expected 896 anchors for face_detection_front, but got {final_anchors.shape[0]}."
            )
...
def _decode_boxes(raw_boxes, anchors):
    ...
    if raw_boxes.shape[0] != anchors.shape[0]:
        raise ValueError(
            f"Shape mismatch: raw_boxes has {raw_boxes.shape[0]} rows, but anchors has {anchors.shape[0]} rows. They must match for decoding."
        )
    ...
    for i in range(num_landmarks):
        landmarks[:, i * 2] = (
            raw_boxes[:, 4 + i * 2] * anchors[:, 2] + anchors[:, 0]
        )  # x_lm
        landmarks[:, i * 2 + 1] = (
...
def non_max_suppression(boxes, scores, threshold):
    ...
    areas = np.maximum(0.0, x2 - x1) * np.maximum(
        0.0, y2 - y1
    )  # Corrected to (x2-x1) without +1
    ...
    while order.size > 0:
        ...
        iou[non_zero_union_mask] = (
            inter[non_zero_union_mask] / union[non_zero_union_mask]
        )
...
def postprocess_blazeface_output(
    ...
    if blaze_face_anchors is None:
        blaze_face_anchors = AnchorBoxes(
            input_size=BLAZEFACE_INPUT_SIZE, min_scale=0.1484375, max_scale=0.75
        )
    ...
    decoded_boxes_normalized, decoded_landmarks_normalized = _decode_boxes(
    ...
    keep_indices = non_max_suppression(
        filtered_boxes_normalized, filtered_scores, iou_threshold
    )
